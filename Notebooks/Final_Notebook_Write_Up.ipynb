 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "The third best model, in our opinion, was the [XGBoost Model](./XGBoost/XG_Boost_1.ipynb). We had used the XGBRegressor package the first time, and had an RMSE of **0.37**, but then we changed the package to XGBClassifier, and the RMSE increased to **0.4206**, which is not an extremely big change looking at it initially, but the values that we are predicting are 1's and 0's. So we would ultimately want the RMSE to be lower than 0.30, as that means we got at least 70% of the predicted values correct for the outcomes, furthermore, an error rate of abou 30% rather than almost 43%, of what it is at at the moment.\n",
    "\n",
